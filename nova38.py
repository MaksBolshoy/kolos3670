#Задача38. Дополнить телефонный справочник возможностью изменения
...
def read_csv(filename: str)-> list:
    ...
    with open(filename, 'r', encoding ='utf-8') as fin:
        for line in fin:
            record = dist(zip(fields, line.strip().split(',')))
            data.append(record)
    return data        


def write_csv(filename: str, data)-> str:
    file = open (filename, 'w',ecoding='utf-8')
    for lines in data:
...
def show_menu() -> int:
    print("\n Выберете необходимое действие:\n"
          ...
          "6.Сохранить спрвочник в текстовом формате\n"
          "7.Закончить работу")
    choice = int(input())
    return choice
...
def delete_user(data, new_record) -> list:
    for key in data:
        if key['Фамилия'].upper() == user_to_delete.upper():
            del key
            print(f'Аббонент{user_to_delete} удален !')


#phone_book = read_csv('Pyton.py.nova38.read_csv')
